  ...
  if (variant === "desktop") {
    ...
    return (
      <nav className="hidden md:flex items-center gap-1">
        {theoryNav.map(renderLink)}
        <span className="mx-1.5 h-5 w-px bg-border/70" aria-hidden />
        {glossaryNav.map(renderLink)}
      </nav>
    );
  }
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-6xl items-stretch justify-around px-1">
        {theoryNav.map(renderTab)}
        <span className="my-2 w-px self-stretch bg-border/70" aria-hidden />
        {glossaryNav.map(renderTab)}
      </div>
    </nav>
  );
}
